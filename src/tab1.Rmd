...
tab1 <- as_tibble(cbind(var = rownames(tab1), tab1)) %>%
  select(var, Missing, `No`, `Yes`, p, SMD)
...
write.xlsx(tab1 %>% select(Variable, Missing:SMD), paste0("./output/tabs/tab1_", Sys.Date(), ".xlsx"), rowNames = FALSE)
...
colnames(tab1) <- sanitize_text(c("Variable", "Missing (%)", "No", "Yes", "p-value", "SMD"))
...
footnote(
  ...
  general = c(
    ...
    "and continuous variables with median [q1-q3] and tested with Kruskal-Wallis test.",
    "SMD are absolute standardized mean differences"
  ),
  symbol = c(
    "Source: SwedeHF",
    "Source: NPR",
